...
## Wiom Design System Override (Active when Pratibimb is loaded)

When working on any Wiom product (Customer App, Partner App, Expert App), the Wiom DS
overrides ALL generic defaults in this skill. Load `wiom-design-system.md` and apply:

**Colors:** Use ONLY DS tokens — never arbitrary hex. brand/600=#D9008D, neutral/white=#FAF9FC.
**Typography:** 22 defined styles only. Noto Sans (EN) + Noto Sans Devanagari (HI). See scale.
**Spacing:** 4px multiples only — 4, 8, 12, 16, 24, 32, 40, 48, 64, 72, 84px.
**Components:** App bar 56px, status bar 44px, CTA 48px height, card radius 12px, icon circles 48×48px.
**Icons:** Google Material + Wiom custom icons. Never set fills on icon instances.
**Post-creation:** Run Pratibimb Design Audit automatically — see `pratibimb.md` for protocol.

Full DS reference: `C:\Users\Abhishek Semwal\.claude\skills\wiom-design-system.md`
